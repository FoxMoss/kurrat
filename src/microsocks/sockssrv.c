...
#endif
#if CONFIG_LOG
/* we log to stderr because it's not using line buffering, i.e. malloc which
   would need locking when called from different threads. for the same reason we
...
static struct addrinfo *addr_choose(struct addrinfo *list,
...
static int connect_socks_target(unsigned char *buf, size_t n,
                                struct client *client) {
  if (n < 5)
...
static int is_authed(union sockaddr_union *client,
                     union sockaddr_union *authedip) {
...
static void send_error(int fd, enum errorcode ec) {
  /* position 4 contains ATYP, the address type, which is the same as used in
     the connect request. we're lazy and return always IPV4 address type in
     errors. */
...
static int usage(void) {
  dprintf(
      2, "MicroSocks SOCKS5 Server\n"
         "------------------------\n"
         "usage: microsocks -1 -q -i listenip -p port -u user -P pass -b "
         "bindaddr -w ips\n"
         "all arguments are optional.\n"
         "by default listenip is 0.0.0.0 and port 1080.\n\n"
         "option -q disables logging.\n"
         "option -b specifies which ip outgoing connections are bound to\n"
         "option -w allows to specify a comma-separated whitelist of ip "
         "addresses,\n"
         " that may use the proxy without user/pass authentication.\n"
         " e.g. -w 127.0.0.1,192.168.1.1.1,::1 or just -w 10.0.0.1\n"
         " to allow access ONLY to those ips, choose an impossible to guess "
         "user/pw combo.\n"
         ...
         " with another program that supports it, and then you can use firefox "
         "too.\n");
  return 1;
}
...
int setup_socks(unsigned port, int (*make_connection)(char *addr, uint16_t port,
  ...
  while (1) {
    ...
    curr->client = c;
    curr->client.make_connection = make_connection;
