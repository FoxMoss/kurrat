...
#define SOCKADDR_UNION_ADDRESS(PTR)                                            \
  ((SOCKADDR_UNION_AF(PTR) == AF_INET)                                         \
       ...
       : ((SOCKADDR_UNION_AF(PTR) == AF_INET6) ? (void *)&(PTR)->v6.sin6_addr  \
                                               : (void *)0))
...
int resolve_sa(const char *host, unsigned short port,
               union sockaddr_union *res);
int bindtoip(int fd, union sockaddr_union *bindaddr);

int server_waitclient(struct server *server, struct client *client);
int server_setup(struct server *server, const char *listenip,
                 unsigned short port);
